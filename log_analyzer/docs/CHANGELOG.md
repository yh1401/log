...
### Added
- **用户维度功能完整实现**：
  - 新增 `user_routes.py` 用户管理路由，支持用户信息和操作日志
  - 前端新增用户信息展示区域，显示用户头像、名称和使用时间
  - 所有用户数据按 user_id 隔离存储，包括对话、上传文件、报告等
  - 请求头统一加入 X-User-Id 和 X-Username 用于身份识别
  - 支持用户操作日志的记录和查询功能

- **文件暂存与合并发送功能**：
  - 上传文件后暂存，不立即发送到对话
  - 用户可以输入提示词或选择开始分析，然后文件和提示词合并发送
  - 优化了请求发送逻辑，确保文件信息完整传递到后端

### Fixed
- **文件和提示词发送但 AI 未识别的问题**：
  - 修复 `chat_manager.py` 中 `stream_message` 方法未正确传递 `files` 参数的问题
  - 修复 `_generate_response` 和 `_generate_stream_response` 方法未接收 `files` 参数的问题
  - 修复 `_try_execute_tools` 方法，新增优先处理暂存文件的逻辑
  - 新增对上传文件和服务器文件的自动检测和分析功能
  - 优化了工具调用流程，确保有暂存文件时优先执行分析
